...
int matrix_multi(int Data_size_in_bits, int Hit_Time_in_clock, int num_p){
	...
	string detail_file_name = "Detail_" + to_string(num_p) + "P" + "_" + to_string(Data_Size_kB) + "kB" + ".csv";
	...
	Detail_file << "Number of Processors" << "," << "Cache Size" << "," << "Ways" << "," << "Words_Per_Block" << "," << "Hit_Time" << ","
		...
		<< endl
		<< num_p << "," << Data_Size_kB / 1024 << "," << Ways << "," << Words_Per_Block << "," << Hit_Time << ","
		<< Lines << "," << Index_Size << "," << Tag_Bits << ","
		<< endl;

	
// Variable for multi-processing
	unsigned int global_clk = 0;	// global clock is used to sync different processor
	unsigned int DRAM_wait_time = 0;	// Time when DRAM is free
	unsigned int* time_resume = new unsigned int[num_p];	// time to resume processing for different processor
...
// Start the global_clk controlled multi-processing, the loop stop after all calculation done
			...
			if (time_resume[p]==global_clk){
				...
				if (proc_state[p]){
					...
					if (hit)
					{
						...
						Detail_file << "global_clk = " << global_clk << "P:" << p << "," << "hit" << ",";

						switch (proc_state[p])
						{
						case 1:	//Read matrix A and hit
							// For matrix A, it is just a read
							time_resume[p] = global_clk + Hit_Time;
							hit_A++;
							
							Detail_file << "A:" << "," << "Address:" << "," << Read_Address << "," << endl;
						...
						case 2: //Read matrix B and hit
							...
							Detail_file << "B:" << "," << "Address:" << "," << Read_Address << "," << endl;
							
							break;
						case 3: //Cache write through for matrix C
							// For matrix C, it is a cache write through, the extra time would be 2 CAS and may be 1 RAS
							if (Previous_RAM_Row_Valid & (Current_RAM_Row == Previous_RAM_Row)){
								time_resume[p] = DRAM_wait_time + (2 * CAS);
							}
							else
							{
								time_resume[p] = DRAM_wait_time + (2 * CAS) + RAS;
								Previous_RAM_Row = Current_RAM_Row;// Update RAM row
							}
							hit_C++;
							DRAM_wait_time = time_resume[p];

							Detail_file << "C:" << "," << "Address:" << "," << Read_Address << "," << endl;
					...
					else
					{
						...
						Detail_file << "global_clk = " << global_clk << "P:" << p << "," << "miss" << ",";
						...
						case 1:	// Matrix A
							...
							if (Previous_RAM_Row_Valid & (Current_RAM_Row == Previous_RAM_Row))
							{
								time_resume[p] = DRAM_wait_time + CAS + 2; // Caculate number of CAS delays needed.
							}
							else // Need RAS and CAS
							{
								Previous_RAM_Row = Current_RAM_Row; // Update RAM row
								time_resume[p] = DRAM_wait_time + CAS + RAS + 2; // Calculate RAS and CAS delays needed.
							}
						...
						case 2:	// Matrix B
							...
							if (Previous_RAM_Row_Valid & (Current_RAM_Row == Previous_RAM_Row))
							{
								time_resume[p] = DRAM_wait_time + CAS + 3; // Caculate number of CAS delays needed.
							}
							else // Need RAS and CAS
							{
								Previous_RAM_Row = Current_RAM_Row; // Update RAM row
								time_resume[p] = DRAM_wait_time + CAS + RAS + 3; // Calculate RAS and CAS delays needed.
							}
							Previous_RAM_Row_Valid = true; // On power up, a RAS is always needed.
						...
						case 3:	// Matrix C
							...
							if (Previous_RAM_Row_Valid & (Current_RAM_Row == Previous_RAM_Row))
							{
								time_resume[p] = DRAM_wait_time + 2*CAS + 2; // Caculate number of CAS delays needed.
							}
							else // Need RAS and CAS
							{
								Previous_RAM_Row = Current_RAM_Row; // Update RAM row
								time_resume[p] = DRAM_wait_time + 2*CAS + RAS + 2; // Calculate RAS and CAS delays needed.
							}
							Previous_RAM_Row_Valid = true; // On power up, a RAS is always needed.
	...
	Result_File << num_p << ","
		<< (double)hit_A / (double)(hit_A + miss_A) * 100 << "," << (hit_A + miss_A) << ","
		<< (double)hit_B / (double)(hit_B + miss_B) * 100 << "," << (hit_B + miss_B) << ","
		<< (double)hit_C / (double)(hit_C + miss_C) * 100 << "," << (hit_C + miss_C) << ","
		<< global_clk << "," << endl;
