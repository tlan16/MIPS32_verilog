...
int matrix_multi(int Data_size_in_bits, int Hit_Time_in_clock, int num_p, string result_file_name, char method){
				...
				if (proc_state[p]){
					...
					else
					{
						...
						case 1:	// Matrix A
							...
							if (Previous_RAM_Row_Valid & (Current_RAM_Row == Previous_RAM_Row))
							{
								time_resume[p] = DRAM_wait_time + CAS + 2; // Calculate number of CAS delays needed.
							}
